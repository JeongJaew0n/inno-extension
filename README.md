...
현재 배포 버전은 [v0.13.0](https://github.com/JeongJaew0n/inno-extension/releases/tag/v0.13.0)이며, 배포 ZIP은 릴리즈 페이지에서 받을 수 있다.
...
기본값은 OFF다. Popup에서 기능을 켜면 보드 업무 상세 모달 헤더에 **전체 폭 토글**이 나타난다.

- 모달은 뷰포트와 무관하게 `1280 × 754`로 고정돼 있어 화면이 넓어도 여백이 남는다.
- Jira에는 이를 넓히는 수단이 없다. 우상단 화살표는 `사이드바로 전환`이라 오히려 좁아지고, `작업` 메뉴에도 전체 화면 항목이 없다.
- 토글을 켜면 폭과 높이 제한을 함께 풀어 브라우저 화면 전체로 넓힌다. 끄면 원래 크기로 돌아온다.
- 백로그의 상세 패널은 대상이 아니다. 그쪽은 모달이 아니라 우측 도킹 패널이다.

#### 백로그 prefix 태그

백로그에서 업무를 추가할 때 입력창 맨 앞에 `/`를 치면 등록된 **prefix 태그** 목록이 나타난다.
...
- `↓` `↑` 로 고르고 `Enter` 또는 `Tab` 으로 넣는다. `Esc` 는 목록만 닫는다.
- 대소문자·대괄호·슬래시를 무시하고 찾는다.
- 문장 중간의 `/` 는 무시한다. 경로나 날짜 입력을 방해하지 않는다.
- 목록이 닫혀 있을 때의 `Enter` 는 평소대로 이슈를 만든다.
...
- 컨트롤 바 아래 줄의 셀렉트에서 **종료된 스프린트**를 고르면 그 업무를 읽기 전용 패널로 보여준다.
- **이 확장에서 Jira API를 쓰는 유일한 기능이다.** 셀렉트를 열기 전에는 요청이 나가지 않고, 읽기(GET)만 한다.
- **보드 카드를 건드리지 않는다.** 패널을 닫으면 원래 보드가 그대로 남고 새로고침이 필요 없다.
- 패널 헤더에서 **전체 보기 · 담당자별 보기**를 바꾼다. 바꿔도 요청이 더 나가지 않는다.
- 카드를 누르면 **Jira 업무 모달**이 열린다. 새 탭도 새로고침도 없다. 드래그·편집은 없다.

#### 활성 스프린트 정보

- 활성 스프린트 보드의 **컨트롤 바 아래 줄**에 **기간 · 남은 일수 · 목표**를 보여준다.
- 목표가 없으면 기간만 나온다.
- **화면이 좁아지면 단계적으로 줄인다** — `기간 · 남음 · 목표` → `기간 · 남음` → `남음` → 아이콘만. 전체 내용은 늘 hover 에 남는다.
- **Jira API 를 부르지 않는다.** 페이지가 이미 들고 있는 값을 읽는다.
- 읽지 못하면 아무것도 보여주지 않는다. 틀린 기간을 띄우지 않는다.

#### 설명 편집 버튼

- 설명을 편집 중일 때 `설명` 제목 옆에 **저장** · **취소** 버튼을 추가한다. 아래쪽 버튼 줄과 순서가 같다. 문서 끝까지 내려가지 않아도 된다.
- 아래쪽 버튼을 그대로 누르므로 동작이 완전히 같다.
- **저장은 되돌릴 수 없어 취소와 다르게 그린다** — 취소는 배경 없는 회색, 저장은 파란 배경이다.
- 읽기 상태에서는 나타나지 않는다.

#### 설명 Markdown 복사

- 업무 **설명** 의 `설명` 라벨 옆에 **Markdown 복사** 버튼을 추가한다.
- 제목 계층, 목록, 표, 링크, 인용, 코드 블록을 Markdown 으로 변환해 평문으로 복사한다.
- **댓글은 복사하지 않는다.** 설명 본문만 대상이다.
- **편집 중에도 복사된다.** 편집 중에는 편집기 내부 문서를 그대로 읽어서 긴 코드 블록도 잘리지 않는다.
- 매크로(Mermaid 등)는 복사하지 않는다. 원본 코드 블록이 그대로 남는다.

#### 설명 Markdown 변환

업무 **설명** 편집기를 열면 toolbar 오른쪽 끝에 **Markdown 변환** 버튼이 나타난다. 기본값은 ON이다 — 버튼은 보이기만 하고 누르기 전에는 아무것도 바꾸지 않는다.
...
현재 릴리즈: [Inno Extension v0.13.0](https://github.com/JeongJaew0n/inno-extension/releases/tag/v0.13.0)
