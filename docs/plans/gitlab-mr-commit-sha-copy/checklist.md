...
- [x] `npm run build` 통과 (typecheck + 테스트 69건)
- [x] 확장 재로드 후 동작 확인 — 2026-08-26 사용자 확인 ("테스트 했어. 잘 되네.")

사용자가 기능 동작을 확인했다. 다만 아래 항목은 개별 측정 결과를 받지 않았으므로 미확인으로
남긴다. 사용자 확인 범위를 넘어 단정하지 않는다.

- [ ] Popup에서 GitLab 토글 동작, 기능 OFF 시 버튼 제거
- [ ] **기존 4개 사이트 회귀 없음** — 사이트 추가는 사후 기록의 장애가 시작된 지점이다.
      `getSettings()` 되쓰기를 제거해 같은 경로는 막혔지만 확인 자체는 별개다.

## 7. 마무리

- [x] 관심사별로 커밋 분리 (구현 `8bdaf46`, 계획 문서 `2cf68c9`)
- [x] 실측 검증 결과를 커밋 메시지와 spec에 반영
- [x] 미검증 항목 명시
- [ ] 릴리즈
