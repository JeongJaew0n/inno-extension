...
| 항목 | 이유 |
| --- | --- |
| Jira에 Mermaid 렌더링을 흉내내기 | 아래 7-1 참조. **2026-09-17 검토 후 하지 않기로 했다** |
| 업무 제목·다른 필드 변환 | 편집기가 아니다 |
| Confluence 기능의 동작 변경 | 공용화는 **동작을 바꾸지 않는 리팩터링**이어야 한다 |

### 7-1. 확장이 직접 이미지로 그리는 안 — 검토했고 하지 않는다

2026-09-17에 실측까지 하고 접었다.

가능한 경로는 하나뿐이다. **외부 서비스는 쓰지 않는다** — 사내 설계문서 본문이 밖으로 나가고,
네트워크 무요청 전제도 깨진다.

```text
mermaid 번들(로컬 렌더) → SVG → canvas → PNG blob
  → 편집기에 붙여넣기 → Jira 가 첨부파일로 업로드
```

data URI 이미지는 ADF `media` 노드가 받지 않고, ADF 에 인라인 SVG 노드는 없다.

**크기 실측** (mermaid 12 설치 후 트리셰이킹한 값)

| 항목 | 크기 |
| --- | --- |
| mermaid 전체 번들 | **7.4 MB** (gzip 1.72 MB) |
| 플로차트+시퀀스만 로드 시 | 1.37 MB (gzip 316 KB) |
| **현재 확장 ZIP 전체** | **164 KB** |

로드는 지연시킬 수 있어도 ZIP 에는 전부 들어간다. **45배가 된다.**

**크기보다 성질이 더 걸린다.**

1. 변환할 때마다 업무에 PNG 첨부파일이 생긴다. 다시 변환하면 또 생긴다
2. 이미지는 편집·검색이 안 되고, 원문을 고치면 다시 변환해야 한다
3. **되돌리기가 깨진다.** 지금까지 이 확장의 모든 동작은 편집 중인 내용만 바꿔서 `취소` 한 번이면
   없던 일이 됐다. 첨부파일 업로드는 그 성질을 깬다

얻는 것은 편집할 수 없는 그림 하나다. **비용이 값어치를 넘는다.**

> 관문 하나는 재지 않았다 — Jira 편집기가 붙여넣은 이미지 blob 을 실제로 업로드하는지. 재는 것
> 자체가 실제 업무에 첨부파일을 남길 수 있어 하지 않았다.
...
| Q3 | 기능 기본값 | ~~OFF~~ → **ON 으로 변경(2026-09-17).** 버튼은 보이기만 하고 누르기 전에는 아무것도 바꾸지 않는다. OFF 면 기능이 있는 줄도 모른다 |
