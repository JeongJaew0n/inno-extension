...
- 발생일: 2026-09-01
- 상태: **대응 적용·확인 완료.** 원인은 유력 가설 단계이며 오류 메시지 전문은 끝내 확보하지 못했다
...
2026-09-01 사용자가 확장 재로드 후 동작을 확인했다. 반복되던 오류가 재현되지 않는다.

다만 **원인이 확정된 것은 아니다.** 오류 메시지 전문을 끝내 확보하지 못했으므로, `Extension context invalidated`였는지 다른 예외였는지는 여전히 모른다. 확인된 것은 다음까지다.

| 항목 | 상태 |
| --- | --- |
| 대응 적용 후 증상이 사라짐 | **확인됨** |
| 원인이 죽은 content script였는지 | **유력하나 미확정** — 메시지 전문 없음 |

증상이 사라진 것은 두 가지 모두와 양립한다. 무효화 감지가 반복 로그를 막았기 때문일 수도 있고, 문제의 탭이 그동안 새로고침되어 죽은 script가 사라졌기 때문일 수도 있다. **둘을 가르는 증거는 없다.**

같은 증상이 다시 나타나면 그때는 메시지 전문이 필요하다. DevTools에서 오류 줄 왼쪽 삼각형을 펼치면 나온다.
