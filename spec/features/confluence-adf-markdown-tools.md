...
현재 범위는 API를 사용하지 않는 로컬 변환 도구다. Confluence `edit-v2` 화면에서 코드블럭 안에 넣은 Markdown 원문이나 문단으로 남은 Markdown을 편집 콘텐츠로 바꾼다. 기존 `본문 Markdown 복사` 기능과 독립적이다.
...
기능이 활성화되면 Confluence `edit-v2` 편집기 toolbar에 `Markdown 변환` 버튼 **하나**를 표시한다. 편집기 toolbar에는 본문 전체를 대상으로 하는 `Markdown -> ADF 변환` 버튼을 표시하지 않는다.

이전에는 `Mermaid -> ADF`와 `코드블럭 -> ADF` 두 버튼으로 나뉘어 있었다. 두 동작은 독립 선택지가 아니라 한 작업의 1단계·2단계다. Markdown 원문의 ` ```mermaid ` 펜스는 코드블럭 벗기기를 거쳐야 개별 Mermaid 코드블럭이 되므로 순서를 바꿀 수 없다. 사용자가 순서를 알아야 하는 UI였기에 하나로 합쳤다.

`Markdown 변환`은 한 번의 클릭에서 세 단계를 순서대로 수행한다.

1. **코드블럭 벗기기** — 조건을 만족할 때만 실행한다
2. **문단 Markdown 변환** — 문단으로 남은 Markdown이 있을 때만 실행한다
3. **Mermaid 변환** — 항상 시도한다

두 단계 모두 대상이 없으면 오류가 아니라 `변환할 내용이 없습니다`로 끝낸다. 다만 그 전에 **변환되지 않고 문단으로 남은 Markdown이 있는지 진단**한다.

#### 미변환 Markdown 진단

원문을 코드블럭이 아니라 본문에 그대로 붙여넣으면 각 줄이 문단이 된다. 이때는 코드블럭이 없어 1단계가 실행되지 않고 Mermaid 코드블럭도 없어 2단계도 할 일이 없다. `변환할 내용이 없습니다`만 보여주면 사용자가 원인을 알 수 없다.

Markdown이 정상 변환되면 문법 문자가 **소비돼 사라진다.** `# 제목`은 `heading` 노드가 되고 텍스트에 `#`이 남지 않는다. 그래서 "문법이 보이는가"가 아니라 **문법 신호와 그것이 되었어야 할 노드를 짝지어** 판정한다.

신호는 **CommonMark·GFM 문법을 실제로 만족하는 줄만** 센다. 느슨하게 매칭하면 사용자가 일부러 쓴 한 줄을 미변환으로 오판한다.

| 구성 | 문법 조건 | 대응 노드 |
| --- | --- | --- |
| 제목 | 들여쓰기 3칸 이하, `#` 1~6개, **뒤에 공백**, 그 뒤에 내용 | `heading` |
| 표 | 한 줄에 파이프(`\|`)를 포함 | `table` |
| 코드블럭 | 들여쓰기 3칸 이하, 백틱 또는 물결 3개 이상 | `codeBlock` |
| 인용 | 들여쓰기 3칸 이하, `>` 뒤에 내용 | `blockquote` |

`#태그`·`#123`은 뒤에 공백이 없어 제목이 아니다. `#######`은 7개라 제목이 아니다. 4칸 들여쓴 줄은 코드 블록이라 제목이 아니다.

대응 노드가 하나라도 있으면 그 구성은 변환된 것으로 보고 **판정을 보류한다.** Markdown 문법을 본문에서 설명하는 문서가 여기 걸린다.

노드가 0이고 문법도 만족하더라도 그것만으로는 확정하지 않는다. 사용자가 일반 문서에 `# TODO` 한 줄을 **일부러** 쓸 수 있다. 확정하려면 다음 증거 중 하나가 필요하다.

| 증거 | 조건 |
| --- | --- |
| 표 구조 | 표 행 **바로 다음 줄**이 구분선이고 **두 줄의 칸 수가 같다** |
| 펜스 짝 | 여는 펜스와 **같은 문자**, **길이 이상**, **info string 없는** 닫는 펜스가 있다 |
| 반복 | 같은 구성의 신호가 **3개 이상** |

제목과 인용에는 짝을 이루는 구조가 없으므로 반복으로만 확정한다.

진단이 확정된 구간은 2단계에서 변환한다. 두 단계 모두 대상이 없고 진단도 걸리지 않으면 `변환할 내용이 없습니다`로 끝낸다.

#### 1단계 실행 조건

코드블럭 벗기기는 코드블럭을 Markdown으로 해석해 산문으로 풀어버린다. 실제 소스 코드에 실행하면 코드가 사라지므로 다음 세 조건을 **모두** 만족할 때만 실행한다.

- 보호 대상(이미 변환된 Mermaid 원본)이 아닌 코드 블록이 하나 이상 있다.
- 그중 본문 **최상위**에 놓인 것이 하나 이상 있다. 목록·인용·표 안의 코드 블록은 문서 구조의 일부로 본다.
- 대상 코드 블록이 **전부** Markdown 문서로 보인다. 하나라도 판정이 서지 않으면 실행하지 않는다.

Markdown 문서 판정은 ATX 제목, 표 구분선, 코드 펜스, 목록, 인용 중 **2종 이상**이 있을 때 참으로 한다. 1종만으로는 오탐이 크다. YAML에는 `- item`이 있고 셸 스크립트에는 `# 주석`이 있다.

원문을 읽지 못한 코드 블록은 Markdown이 아닌 것으로 본다. 확신이 없을 때 벗기면 내용을 잃기 때문이다.

최상위 판정은 Confluence가 코드 블록을 감싸는 `.fabric-editor-breakout-mark` 래퍼를 통과해야 한다. 편집기의 직계 자식만 보면 코드 블록이 하나도 잡히지 않는다.

#### 1단계 동작

실행 조건을 만족하면 대상 범위는 종전 `코드블럭 -> ADF`와 같다. 현재 본문 안의 Confluence 코드 블록을 대상으로 한다.
...
- 진행 상황을 단계와 개수로 표시한다(`코드블럭 2/3`, `Mermaid 3/7`). 후보당 최대 3초가 걸려 전체가 20초를 넘길 수 있으므로 멈춘 것처럼 보이지 않아야 한다.
- 결과는 수행한 단계만 표시한다(`코드블럭 1 · Mermaid 7 변환`). 부분 실패는 `· 일부 실패`를 덧붙인다.
- 단계 사이에서 실패해도 전역 되돌리기를 하지 않는다. Confluence 실행 취소는 transaction 단위라 여러 단계를 한 번에 되돌릴 수 없고, 반복 실행하면 사용자의 다른 편집까지 지운다.
- 확장은 `업데이트` 버튼을 누르지 않으며, 사용자는 저장 전 결과를 검토하고 Confluence 실행 취소로 되돌릴 수 있다.

#### 2단계 — 문단 Markdown 변환

본문 최상위의 **연속한 문단** 구간을 모아, 문법 증거가 있는 구간만 대상으로 한다. Confluence가 붙여넣기 과정에서 이미 변환해 둔 노드(목록·표 등)는 문단이 아니므로 구간을 끊으며 건드리지 않는다.

변환은 구간의 문단 텍스트를 그대로 이어 붙여 **평문으로 다시 붙여넣는 방식**이다. 그러면 Confluence 자체 Markdown 파서가 제목·표·코드블럭·목록·인용을 만든다.

이 단계에서는 **우리 변환기를 쓰지 않는다.** 두 파서의 규칙이 다르기 때문이다.

| 항목 | Confluence 파서 | `marked` (우리 변환기) |
| --- | --- | --- |
| 취소선 구분자 | `~~` 만 | `~` 하나도 인정 |
| `1~3장 ... 4~5장` | 그대로 유지 | 취소선이 되고 물결표가 사라짐 |

사용자가 직접 붙여넣었을 때와 같은 결과를 내는 쪽이 맞다. 클립보드에는 `text/plain`만 싣는다. `text/html`이 함께 있으면 Confluence가 그쪽을 우선해 Markdown 파서를 타지 않는다.

구간은 **뒤에서부터** 처리해 앞 구간 교체가 뒤 구간의 위치를 어긋내지 않게 한다. 교체 성공은 노드 동일성이 아니라 **문법이 남은 줄이 사라졌는지**로 판정한다. ProseMirror가 문단 DOM 노드를 재사용해 원래 엘리먼트의 연결 상태가 유지되기 때문이다.

#### 3단계 동작

3단계는 현재 본문의 코드 블록 중 첫 유효 선언이 Mermaid diagram type인 블록만 대상으로 한다.

앞 단계가 실행됐다면 코드 블록의 순번과 노드 참조가 모두 바뀌므로, 3단계는 편집 본문을 **다시 조회한 뒤** 시작한다. 매크로의 `guestParams.index`도 3단계 시작 시점 기준으로 다시 센다.

짝을 찾지 못한 기존 Mermaid 컴포넌트 검사는 **1단계보다 먼저** 수행한다. 1단계가 문서를 바꾼 뒤에 중단하면 되돌리기 곤란하기 때문이다.
...
코드블럭 벗기기 결과에서 Mermaid fenced code block은 Mermaid 앱 매크로가 아니라 **최상위 `codeBlock(language: mermaid)`**로 보존한다. 변환기는 tenant나 페이지 문맥을 모르기 때문에 서드파티 Mermaid 앱 렌더링을 보장하지 않는다.

이전에는 `expand('Mermaid 코드 보기')`로 감쌌으나, Mermaid 변환이 최상위 코드 블록만 교체를 검증할 수 있어 후속 변환이 100% 실패했다. 원본을 접는 것은 편집기 변환이 `Mermaid 원본` 영역으로 직접 하므로, 다이어그램이 없는 시점에 미리 접지 않는다.
...
- 2026-09-04: 31줄 이상인 코드블럭이 Mermaid 변환 검증을 통과하지 못하던 문제를 고쳤다. 원문은 브리지로 ProseMirror에서 전체를 읽는데 검증은 DOM에서 읽어 비교했고, CodeMirror가 30줄 안팎까지만 렌더해 두 값이 영원히 달랐다. 렌더된 구간이 원문의 연속 부분이면 통과하도록 비교를 완화했다. 아무것도 렌더되지 않은 코드블럭은 통과시키지 않는다.
- 2026-09-04: 편집기 toolbar의 버튼 2개(`Mermaid -> ADF`, `코드블럭 -> ADF`)를 `Markdown 변환` 하나로 합쳤다. 두 동작은 순서가 고정된 1단계·2단계인데 UI가 나뉘어 있어 사용자가 순서를 알아야 했다. 코드블럭 벗기기는 **대상이 전부 Markdown 문서로 보일 때만** 실행하도록 조건을 걸었다. 종전에는 실제 소스 코드까지 산문으로 풀어버렸다.
- 2026-09-04: 변환되지 않고 문단으로 남은 Markdown을 진단하고 변환한다. 문법 신호와 대응 노드를 짝지어 판정하되, 신호는 CommonMark·GFM 문법을 실제로 만족하는 줄만 세고, 노드가 0이어도 표 헤더·구분선의 칸 수 일치, 규칙대로 닫힌 펜스, 3회 이상 반복 중 하나가 있어야 확정한다. 변환은 구간을 평문으로 다시 붙여넣어 Confluence 자체 파서에 맡긴다. 우리 변환기는 취소선 구분자로 물결표 1개도 인정해 `1~3` 같은 범위 표기를 깨뜨리므로 이 경로에서는 쓰지 않는다.
- 2026-09-04: Popup의 Markdown -> ADF JSON 변환기를 제거했다. 결과가 읽기 전용 JSON이라 문서에 넣을 방법이 없었고 편집기 버튼과 역할이 겹쳤다. 게다가 Popup은 `marked`, 편집기 문단 변환은 Confluence 파서를 쓰는데 취소선 구분자 규칙이 서로 달라 미리보기가 실제 결과와 어긋났다. 기능 상세에는 편집기 안내만 남긴다.
