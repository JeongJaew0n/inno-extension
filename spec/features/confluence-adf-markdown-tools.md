...
`Markdown 변환`은 한 번의 클릭에서 두 단계를 순서대로 수행한다.

1. **코드블럭 벗기기** — 조건을 만족할 때만 실행한다
2. **Mermaid 변환** — 항상 시도한다

두 단계 모두 대상이 없으면 오류가 아니라 `변환할 내용이 없습니다`로 끝낸다.
...
2단계는 현재 본문의 코드 블록 중 첫 유효 선언이 Mermaid diagram type인 블록만 대상으로 한다.

1단계가 실행됐다면 코드 블록의 순번과 노드 참조가 모두 바뀌므로, 2단계는 편집 본문을 **다시 조회한 뒤** 시작한다. 매크로의 `guestParams.index`도 2단계 시작 시점 기준으로 다시 센다.
