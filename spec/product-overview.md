...
- **API는 최대한 쓰지 않는다.** DOM으로 안 되는 것만 쓰고, 그런 기능은 카탈로그에 `usesNetwork`로 표시해 특별 관리한다. 읽기(GET)만 하고, 사용자가 그 기능을 실제로 쓸 때만 요청하며, 그 사이트 오리진으로만 보낸다. 토큰을 저장하지 않고 브라우저 세션을 그대로 쓴다. 표시되지 않은 기능에서 네트워크 호출이 발견되면 테스트가 깨진다.
...
- 2026-09-18: Jira 보드에 `지난 스프린트 보기`를 추가했다. **이 확장에서 Jira API를 쓰는 첫 기능이다.** 그래서 제품 전제를 `네트워크 요청을 보내지 않는다`에서 `API를 쓸 수 있지만 최대한 쓰지 않고, 쓰는 기능은 특별 관리한다`로 바꿨다. 특별 관리는 문장이 아니라 **테스트로 막았다** — 카탈로그에 표시되지 않은 자리에서 네트워크 호출이 발견되면 깨진다. 보드 카드는 건드리지 않고 읽기 전용 패널을 덮는다. 카드를 바꾸면 React 가상 DOM과 어긋나 드래그나 `스프린트 완료`가 보이는 것과 다른 대상에 걸리기 때문이다.
