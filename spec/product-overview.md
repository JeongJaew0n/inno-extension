...
- 2026-09-17: Jira 설명을 편집 중일 때 `설명` 제목 옆에 `취소`·`저장` 버튼을 추가했다. 설명은 본문을 클릭하면 곧바로 편집 상태가 되는데 되돌리려면 문서 끝까지 내려가야 했다. **아래쪽 취소 버튼을 그대로 누른다** — 확인 흐름을 흉내 내지 않는다. 그 버튼의 `data-testid`가 `comment-cancel-button`·`comment-save-button`이라 진짜 댓글 입력창의 것과 겹치므로 설명 편집 컨테이너 안에서만 찾는다. 저장은 되돌릴 수 없어 취소와 눈에 띄게 다르게 그린다. 기본값은 ON이다.
