...
- 2026-09-17: Jira 업무 **설명** 편집기에 `Markdown 변환`을 추가했다. Confluence 편집기 기능의 두 단계(코드블럭 벗기기·문단 Markdown 변환)를 `platform/editor/`로 공용화해 붙였다. 같은 Atlassian ADF 편집기라 그대로 동작하는 것을 실측으로 확인했다. **Mermaid 단계는 제외했다** — Jira에는 Mermaid 앱이 설치돼 있지 않아 같은 노드를 넣어도 그릴 주체가 없다. Jira는 설명과 댓글 편집기가 동시에 열릴 수 있어 설명 컨테이너 안에서만 툴바를 찾는다. 기본값은 OFF다.
