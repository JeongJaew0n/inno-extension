...
- 2026-09-16: Jira에 `백로그 prefix 태그` 기능을 추가했다. 백로그에서 업무를 추가할 때 `/`를 입력하면 등록된 prefix 태그 목록이 뜨고 골라서 넣는다. 실제 백로그 제목 37개에 16종이 쓰이고 있어 손으로 치면 표기가 흔들렸다. 내장 태그 아홉 개를 제공하며 숨길 수는 있어도 삭제할 수 없고, 사용자는 커스텀 태그를 추가·수정·삭제할 수 있다. 이 입력창의 Enter가 이슈 생성이므로 목록이 열렸을 때만 가로챈다.
