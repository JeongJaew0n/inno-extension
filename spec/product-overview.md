...
- 2026-09-16: Jira에 `백로그 슬래시 템플릿` 기능을 추가했다. 백로그에서 업무를 추가할 때 `/`를 입력하면 미리 등록한 제목 접두사 목록이 뜨고 골라서 넣는다. 실제 백로그 제목 37개에 접두사가 16종 쓰이고 있어 손으로 치면 표기가 흔들렸다. 템플릿은 Popup에서 한 줄에 하나씩 편집한다. 이 입력창의 Enter가 이슈 생성이므로 목록이 열렸을 때만 가로챈다.
