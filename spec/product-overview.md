...
- service worker는 **설정 창을 여는 일 하나만** 한다. content script가 쓸 수 없는 `chrome.windows`를 대신 부르는 자리이며, 그 외의 로직을 여기에 두지 않는다. 새 권한도 쓰지 않는다.
...
- 2026-09-04: Confluence 편집기의 변환 버튼 2개를 `Markdown 변환` 하나로 합치고, 문단으로 남은 Markdown을 변환하는 단계를 추가했다. 코드블럭 벗기기는 대상이 전부 Markdown 문서로 보일 때만 실행해 실제 소스 코드를 산문으로 풀어버리던 문제를 막는다. 문단 변환은 Confluence 자체 파서에 맡겨 사용자가 직접 붙여넣었을 때와 같은 결과를 낸다. **Popup의 Markdown -> ADF JSON 변환기는 제거했다** — 결과를 문서에 넣을 방법이 없어 편집기 버튼과 역할이 겹쳤고, 두 경로의 Markdown 해석 규칙이 달라 미리보기가 실제 결과와 어긋났다.
- 2026-09-15: Jira에 `업무 모달 전체 폭` 기능을 추가했다. 보드 업무 상세 모달이 뷰포트와 무관하게 `1280 × 754`로 고정돼 있어 화면이 넓어도 여백이 남았다. Jira에는 이를 넓히는 수단이 없다 — 우상단 화살표는 `사이드바로 전환`이라 오히려 좁아지고, `작업` 메뉴에도 전체 화면 항목이 없다. 모달 헤더에 토글을 넣어 폭과 높이 제한을 함께 푼다. 기본값은 OFF다.
- 2026-09-15: 프로젝트 규칙을 `CLAUDE.md`에 명시했다. 커밋 author를 `git config --local`로 고정하고, 커밋·푸시·브랜치 정책과 0.x 버전 규칙, 기능 묶음 단위(사이트)를 기록했다. `docs/glossary/`와 `docs/troubleshootings/`를 추가했다. 공개 저장소에 개인 이메일로 올라가 있던 과거 커밋 전체의 author를 GitHub noreply로 재작성하고, 이전 릴리즈와 태그를 모두 정리했다.
- 2026-09-18: Jira 활성 스프린트 보드 상단 `빠른 필터` 옆에 스프린트 기간·남은 일수·목표를 표시한다. 보드 화면 어디에도 이 값이 없어 `스프린트 세부 정보` 팝업을 매번 열어야 했다. **API를 부르지 않는다** — 페이지가 이미 들고 있는 `window.SPA_STATE`를 읽기 전용 MAIN world 브리지로 가져온다. 상태를 읽지 못하면 조용히 빠진다. 기본값은 ON이다.
- 2026-09-17: Jira 설명을 편집 중일 때 `설명` 제목 옆에 `저장`·`취소` 버튼을 추가했다. 설명은 본문을 클릭하면 곧바로 편집 상태가 되는데 되돌리려면 문서 끝까지 내려가야 했다. **아래쪽 취소 버튼을 그대로 누른다** — 확인 흐름을 흉내 내지 않는다. 그 버튼의 `data-testid`가 `comment-cancel-button`·`comment-save-button`이라 진짜 댓글 입력창의 것과 겹치므로 설명 편집 컨테이너 안에서만 찾는다. 저장은 되돌릴 수 없어 취소와 눈에 띄게 다르게 그린다. 기본값은 ON이다.
- 2026-09-17: Jira 업무 **설명**에 `Markdown 복사`를 추가했다. Jira 설명 읽기 화면이 Confluence 문서 본문과 같은 `.ak-renderer-document`라 변환기를 `platform/editor/`로 공용화해 그대로 썼다. 댓글도 같은 렌더러를 쓰므로 설명 필드 안에서만 본문을 찾는다. 기본값은 ON이다.
- 2026-09-17: Jira 업무 **설명** 편집기에 `Markdown 변환`을 추가했다. Confluence 편집기 기능의 두 단계(코드블럭 벗기기·문단 Markdown 변환)를 `platform/editor/`로 공용화해 붙였다. 같은 Atlassian ADF 편집기라 그대로 동작하는 것을 실측으로 확인했다. **Mermaid 단계는 제외했다** — Jira에는 Mermaid 앱이 설치돼 있지 않아 같은 노드를 넣어도 그릴 주체가 없다. Jira는 설명과 댓글 편집기가 동시에 열릴 수 있어 설명 컨테이너 안에서만 툴바를 찾고, 버튼은 툴바 오른쪽 끝에 둔다. 기본값은 ON이다 — 버튼은 보이기만 하고 누르기 전에는 아무것도 바꾸지 않으며 저장도 사용자가 누른다.
- 2026-09-16: Jira에 `백로그 prefix 태그` 기능을 추가했다. 백로그에서 업무를 추가할 때 `/`를 입력하면 등록된 prefix 태그 목록이 뜨고 골라서 넣는다. 실제 백로그 제목 37개에 16종이 쓰이고 있어 손으로 치면 표기가 흔들렸다. 내장 태그 아홉 개를 제공하며 숨길 수는 있어도 삭제할 수 없고, 사용자는 커스텀 태그를 추가·수정·삭제할 수 있다. 이 입력창의 Enter가 이슈 생성이므로 목록이 열렸을 때만 가로챈다. 목록 맨 아래의 `추가·관리` 버튼을 마우스로 누르면 설정 화면이 창으로 뜬다 — 이를 위해 설정 창을 여는 일만 하는 service worker를 처음으로 두었다.
